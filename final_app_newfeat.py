...
def create_schedule_section():
    ...
    with st.form("weekly_goals_form"):
        st.write("Enter your weekly goals for each priority (in hours):")
        career_goal = st.number_input("Career", min_value=0.0, step=0.25)
        music_goal = st.number_input("Music", min_value=0.0, step=0.25)
        fitness_goal = st.number_input("Fitness", min_value=0.0, step=0.25)
        relationship_goal = st.number_input("Relationship", min_value=0.0, step=0.25)
        philosophy_goal = st.number_input("Philosophy", min_value=0.0, step=0.25)
        finance_goal = st.number_input("Finance", min_value=0.0, step=0.25)
        
        if st.form_submit_button("Save Weekly Goals"):
            # Save weekly goals to Google Sheets or session state
            st.success("Weekly goals saved!")

def calculate_daily_allocations(weekly_goals):
    daily_allocations = {priority: goal / 7 for priority, goal in weekly_goals.items()}
    return daily_allocations

def display_progress(weekly_goals, actual_hours):
    for priority, goal in weekly_goals.items():
        progress = (actual_hours.get(priority, 0) / goal) * 100
        st.progress(progress)
        st.write(f"{priority}: {actual_hours.get(priority, 0):.1f} / {goal:.1f} hours")

def merge_planned_vs_actual(schedule_df, logged_df):
    # Convert 'Date' column to datetime and handle timezone
    schedule_df['Date'] = pd.to_datetime(schedule_df['Date'])
    if schedule_df['Date'].dt.tz is None:  # Check if timezone-naive
        schedule_df['Date'] = schedule_df['Date'].dt.tz_localize(IST)  # Localize to IST
    else:  # If already timezone-aware, convert to IST
        schedule_df['Date'] = schedule_df['Date'].dt.tz_convert(IST)
    
    # Convert 'Timestamp' column to datetime and handle timezone
    logged_df['Timestamp'] = pd.to_datetime(logged_df['Timestamp'])
    if logged_df['Timestamp'].dt.tz is None:  # Check if timezone-naive
        logged_df['Timestamp'] = logged_df['Timestamp'].dt.tz_localize(IST)  # Localize to IST
    else:  # If already timezone-aware, convert to IST
        logged_df['Timestamp'] = logged_df['Timestamp'].dt.tz_convert(IST)
    
    # Extract date from 'Timestamp' in logged_df
    logged_df['Date'] = logged_df['Timestamp'].dt.date
    logged_df['Date'] = pd.to_datetime(logged_df['Date']).dt.tz_localize(IST)  # Localize to IST
...
def create_heatmap(comparison_df):
    fig = px.imshow(
        comparison_df.pivot(index='Date', columns='Priority', values='Difference'),
        labels=dict(x="Priority", y="Date", color="Difference"),
        title="Planned vs Actual Hours (Heatmap)"
    )
    st.plotly_chart(fig)

def create_cumulative_chart(comparison_df):
    comparison_df['Cumulative_Planned'] = comparison_df.groupby('Priority')['Planned_Duration'].cumsum()
    comparison_df['Cumulative_Actual'] = comparison_df.groupby('Priority')['Duration_actual'].cumsum()
    
    fig = px.line(
        comparison_df,
        x='Date',
        y=['Cumulative_Planned', 'Cumulative_Actual'],
        color='Priority',
        title="Cumulative Planned vs Actual Hours"
    )
    st.plotly_chart(fig)

def create_radar_chart(kpi_df):
    fig = px.line_polar(
        kpi_df,
        r='Total_Actual_Hours',
        theta='Priority',
        line_close=True,
        title="Priority Balance (Radar Chart)"
    )
    st.plotly_chart(fig)

# Habit formulation and reminders

def show_reminders(schedule):
    now = datetime.now(IST).time()
    for activity in schedule:
        if activity['start_time'] <= now <= activity['end_time']:
            st.toast(f"Time for {activity['priority']}: {activity['activity']}")
...
def show_reflection_prompt():
    if datetime.now(IST).hour >= 21:  # Show prompt after 9 PM
        st.write("How well did you follow your schedule today?")
...
def main():
    ...
    with tab3:
        ...
        # Add Weekly Goals and Progress Tracking
        st.subheader("Weekly Goals and Progress")
        weekly_goals = {
            "Career": st.number_input("Career Weekly Goal (hours)", min_value=0.0, step=0.25, value=20.0),
            "Music": st.number_input("Music Weekly Goal (hours)", min_value=0.0, step=0.25, value=10.0),
            "Fitness": st.number_input("Fitness Weekly Goal (hours)", min_value=0.0, step=0.25, value=10.0),
            "Relationship": st.number_input("Relationship Weekly Goal (hours)", min_value=0.0, step=0.25, value=5.0),
            "Philosophy": st.number_input("Philosophy Weekly Goal (hours)", min_value=0.0, step=0.25, value=5.0),
            "Finance": st.number_input("Finance Weekly Goal (hours)", min_value=0.0, step=0.25, value=5.0),
        }
        
        if st.button("Save Weekly Goals"):
            st.session_state.weekly_goals = weekly_goals
            st.success("Weekly goals saved!")
        
        if 'weekly_goals' in st.session_state:
            st.write("### Daily Allocations")
            daily_allocations = calculate_daily_allocations(st.session_state.weekly_goals)
            for priority, allocation in daily_allocations.items():
                st.write(f"{priority}: {allocation:.2f} hours/day")
            
            st.write("### Progress Tracking")
            df = load_data("Sheet1")
            actual_hours = df.groupby('Priority')['Duration'].sum().to_dict()
            display_progress(st.session_state.weekly_goals, actual_hours)
    ...
    with tab4:
        ...
        if 'Duration' in logged_df.columns and 'Planned_Duration' in schedule_df.columns:
            # Merge planned and actual hours
            comparison_df = merge_planned_vs_actual(schedule_df, logged_df)
            
            # Create Heatmap
            st.write("### Daily Heatmap")
            create_heatmap(comparison_df)
            
            # Create Cumulative Progress Chart
            st.write("### Cumulative Progress")
            create_cumulative_chart(comparison_df)
            
            # Create Radar Chart
            st.write("### Priority Balance")
            kpi_df = calculate_kpis(comparison_df)
            create_radar_chart(kpi_df)
        else:
            st.error("Required columns are missing in the data.")
    
    # Habit Formation and Reminders
    if 'schedule' in st.session_state:
        show_reminders(st.session_state.schedule)
    
    # Reflection Prompt at the end of the day
